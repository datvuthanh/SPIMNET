...
save_path = '/home/memi/Desktop/SPIMNET/patches_for_training/neg_new_10/'
...
for record in records:
    ...
    default_x = random.randint(200, 300)
    default_y = random.randint(100, 300)
    # Shift the patches in x-direction for making negative pairs
    var = random.randint(-16, 16)
    while (var == 0 or var == 1 or var == 2 or var == 3 or var == -1 or var == -2 or var == -3):
        var = random.randint(-16, 16)
    ...
    if left_disp1 == right_disp1:
        ...
        right_patch = tmp_nir[list_y[0]-size:list_y[0]+size, list_x[0]-left_disp1+var-size:list_x[0]-left_disp1+var+size, :]
    ...
    if left_disp2 == right_disp2:
        ...
        right_patch = tmp_nir[list_y[1]-size:list_y[1]+size, list_x[1]-left_disp2+var-size:list_x[1]-left_disp2+var+size, :]
        imgs = [left_patch, right_patch] 
        imgs_comb = np.hstack(imgs[i] for i in range(2))
        imgs_comb = Image.fromarray(imgs_comb)
        path_save  = save_path + str(cnt) + '.png'
    ...
    if left_disp3 == right_disp3:
        ...
        right_patch = tmp_nir[list_y[2]-size:list_y[2]+size, list_x[2]-left_disp3+var-size:list_x[2]-left_disp3+var+size, :]
        imgs = [left_patch, right_patch] 
        imgs_comb = np.hstack(imgs[i] for i in range(2))
        imgs_comb = Image.fromarray(imgs_comb)
        path_save  = save_path + str(cnt) + '.png'
